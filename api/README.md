...
- Backtest run metadata, event history, and completed results are persisted in a local SQLite experiment registry so `GET /v1/backtests` still shows historical runs after restart.
...
- `GLOWBACK_API_KEY` enables API-key authentication (`Authorization: Bearer <token>` or `X-API-Key: <token>`). WebSocket clients can pass `?api_key=<token>` when headers are unavailable.
